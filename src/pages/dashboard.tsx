...
import { useRoqPlatformUserProfiles, useTheaterCount, useShowCount, useBookingCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const theaterStat = useTheaterCount();

  const showStat = useShowCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Theaters',
      result: theaterStat?.data,
      isLoading: theaterStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Shows',
      result: showStat?.data,
      isLoading: showStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bookings',
