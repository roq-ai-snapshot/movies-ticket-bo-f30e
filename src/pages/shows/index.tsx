...
import { useRoqClient, useShowFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ShowInterface, unknown>;
...
export function ShowListPage(props: ShowListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['movie', 'theater', 'booking.count'],
  ...
  const { data, error, isLoading, mutate } = useShowFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.show.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ShowInterface) => {
    if (hasAccess('show', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/shows/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) => (record?.show_time ? format(record?.show_time, 'dd-MM-yyyy') : ''),
    },
    { id: 'ticket_price', header: 'ticket_price', accessorKey: 'ticket_price' },
    ...
    hasAccess('theater', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'theater',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/theaters/view/${record.theater?.id}`}>
              {record.theater?.name}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/shows/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('show', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/shows/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('show', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="show" />
        </Flex>

        {hasAccess('show', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/shows/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
